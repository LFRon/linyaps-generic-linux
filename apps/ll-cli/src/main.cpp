    ...
    cliImportCdi
      ->add_option("--config",
                   extensionOptions.configPath,
                                     _("Specify config path (default: ~/.config/linglong/extension_overrides.json)"))
      ->type_name("FILE");
    cliImportCdi
      ->add_option("--cdi",
...
{
    return std::string{
        _("  ll-cli config add-ext        [--global | --app <appid>] --target TARGET_REF EXT_REF [EXT_REF ...]\n"
           "  ll-cli config rm-ext         [--global | --app <appid>] --target TARGET_REF EXT_REF [EXT_REF ...]\n"
           "  ll-cli config clear-ext      [--global | --app <appid>] --target TARGET_REF\n"
           "  ll-cli config set-env        [--global | --app <appid>] KEY=VAL [KEY=VAL ...]\n"
           "  ll-cli config unset-env      [--global | --app <appid>] KEY [KEY ...]\n"
           "  ll-cli config add-fs         [--global | --app <appid>] --host PATH --target PATH "
           "[--mode ro|rw] [--persist]\n"
           "  ll-cli config rm-fs          [--global | --app <appid>] (--target PATH | --index N)\n"
           "  ll-cli config add-fs-allow   [--global | --app <appid>] --host PATH --target PATH "
           "[--mode ro|rw] [--persist]\n"
           "  ll-cli config rm-fs-allow    [--global | --app <appid>] (--target PATH | --index N)\n"
           "  ll-cli config clear-fs-allow [--global | --app <appid>]\n"
           "  ll-cli config set-command    [--global | --app <appid>] <cmd> [--entrypoint P] [--cwd D] "
           "[--args-prefix \"...\"] [--args-suffix \"...\"] [KEY=VAL ...]\n"
           "  ll-cli config unset-command  [--global | --app <appid>] <cmd>\n"
           "  ll-cli config enable-permission  [--global | --app <appid>] --category NAME PERM [PERM ...]\n"
           "  ll-cli config disable-permission [--global | --app <appid>] --category NAME PERM [PERM ...]\n"
           "  ll-cli config add-udev-rule       [--global | --app <appid>] --name NAME --file PATH\n"
           "  ll-cli config rm-udev-rule        [--global | --app <appid>] --name NAME\n"
...
static std::filesystem::path getConfigPath(Scope scope, const std::string &appId)
{
    return buildConfigPath(getBaseConfigDir(), scope, appId);
}
...
static bool resolveScopeOptions(const ConfigScopeOptions &opts,
                                ...
                                std::string &error)
{
    int count = (opts.global ? 1 : 0) + (!opts.appId.empty() ? 1 : 0);
    if (count != 1) {
        error = "specify exactly one of --global or --app <appid>";
        return false;
    }
    if (opts.global) {
        scope = Scope::Global;
...
static std::optional<json> openConfig(Scope scope, const std::string &appId)
{
    auto userPath = getConfigPath(scope, appId);
    if (userPath.empty()) {
        fprintf(stderr, "invalid config path\n");
        return std::nullopt;
    }
    auto userJson = readJsonIfExists(userPath, nullptr);
    if (!userJson) {
...
static bool saveConfig(Scope scope,
                       ...
                       const json &j)
{
    auto path = getConfigPath(scope, appId);
    if (path.empty()) {
        return false;
    }
...
int runCliApplication(int argc, char **mainArgv)
{
    ...
    auto resolveScopeOrThrow = [&](const ConfigScopeOptions &opts) {
        ...
        std::string appId, error;
        if (!resolveScopeOptions(opts, scope, appId, error)) {
            throw CLI::ValidationError("scope", error);
        }
        return std::make_tuple(scope, appId);
    };
    ...
    auto ensureExtDefs = [](json &root) -> json & {
        auto &node = root["ext_defs"];
        if (!node.is_object()) {
            node = json::object();
        }
        return node;
    ...
    auto extDefFromRefOrThrow = [](const std::string &ref) -> json {
        auto fuzzyRef = FuzzyReference::parse(ref);
        if (!fuzzyRef) {
            throw CLI::ValidationError("EXT_REF",
                                       "failed to parse extension ref: " + fuzzyRef.error().message());
        }
        json ext = json::object();
        ext["directory"] = "/opt/extensions/" + fuzzyRef->id;
        ext["name"] = fuzzyRef->id;
        ext["version"] = fuzzyRef->version.value_or("");
        return ext;
    };

    auto addExtToTarget = [&](json &root, const std::string &target, const json &ext) {
        auto &extDefs = ensureExtDefs(root);
        auto &arr = extDefs[target];
        if (!arr.is_array()) {
            arr = json::array();
        }
        for (const auto &existing : arr) {
            if (existing.is_object() && existing == ext) {
                return;
            }
        }
        arr.push_back(ext);
    ...
    auto removeExtFromTarget = [&](json &root, const std::string &target, const json &ext) -> bool {
        auto &extDefs = ensureExtDefs(root);
        auto it = extDefs.find(target);
        if (it == extDefs.end() || !it->is_array()) {
            return false;
        }
        auto &arr = *it;
    ...
    // config add-ext
    {
        auto *sub = configCmd->add_subcommand(
          "add-ext",
          _("Add extension definition(s) into ext_defs for a target ref (global/app config only)"));
        ...
        auto refs = std::make_shared<std::vector<std::string>>();
        sub->add_option("--target", *target, _("Target ref key used by ext_defs (e.g. stable:org.app/1.0.0)"))
          ->required();
        sub->add_option("EXT_REF", *refs, _("Extension ref(s), e.g. stable:org.ext/1.0.0"))
          ->required()
          ->expected(1, -1);
        sub->callback([&, scopeOpts, target, refs]() {
            auto [scope, appId] = resolveScopeOrThrow(*scopeOpts);
            auto j = openConfig(scope, appId);
            if (!j) {
                handleConfigResult(false);
                return;
            }
            for (const auto &ref : *refs) {
                addExtToTarget(*j, *target, extDefFromRefOrThrow(ref));
            }
            handleConfigResult(saveConfig(scope, appId, *j));
        });
    }
    ...
    // config rm-ext
    {
        auto *sub = configCmd->add_subcommand(
          "rm-ext",
          _("Remove extension definition(s) from ext_defs for a target ref (global/app config only)"));
        auto scopeOpts = std::make_shared<ConfigScopeOptions>();
        addConfigScopeOptions(sub, *scopeOpts);
        auto target = std::make_shared<std::string>();
        auto refs = std::make_shared<std::vector<std::string>>();
        sub->add_option("--target", *target, _("Target ref key used by ext_defs"))->required();
        sub->add_option("EXT_REF", *refs, _("Extension ref(s) to remove"))->required()->expected(1, -1);
        sub->callback([&, scopeOpts, target, refs]() {
            auto [scope, appId] = resolveScopeOrThrow(*scopeOpts);
            auto j = openConfig(scope, appId);
            if (!j) {
            ...
            for (const auto &ref : *refs) {
                removedAny = removeExtFromTarget(*j, *target, extDefFromRefOrThrow(ref)) || removedAny;
            }
            if (!removedAny) {
                fprintf(stderr, "no ext_defs entry removed\n");
                handleConfigResult(false);
                return;
            }
            handleConfigResult(saveConfig(scope, appId, *j));
    ...
    // config clear-ext
    {
        auto *sub = configCmd->add_subcommand("clear-ext",
                                              _("Clear ext_defs entries for a target ref"));
        auto scopeOpts = std::make_shared<ConfigScopeOptions>();
        addConfigScopeOptions(sub, *scopeOpts);
        auto target = std::make_shared<std::string>();
        sub->add_option("--target", *target, _("Target ref key used by ext_defs"))->required();
        sub->callback([&, scopeOpts, target]() {
            auto [scope, appId] = resolveScopeOrThrow(*scopeOpts);
            auto j = openConfig(scope, appId);
            if (!j) {
                handleConfigResult(false);
            ...
            auto &extDefs = ensureExtDefs(*j);
            bool erased = (extDefs.erase(*target) > 0);
            if (!erased) {
                fprintf(stderr, "no ext_defs key removed\n");
                handleConfigResult(false);
                return;
            }
            handleConfigResult(saveConfig(scope, appId, *j));
