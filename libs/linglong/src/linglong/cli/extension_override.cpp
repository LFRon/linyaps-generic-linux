...
std::optional<std::filesystem::path> getUserConfigPath() noexcept
{
    ...
    // Keep extension overrides isolated from ll-cli runtime config to avoid
    // accidental overwrites and schema conflicts.
    return base / "linglong" / "extension_overrides.json";
}
